...
function PlayerStandard:_check_action_interact(t, input)
    if SYSHUD._settings.interaction then
        if input.btn_interact_press and self:_interacting() then
            self:_interupt_action_interact()
            return false
        elseif input.btn_interact_release and self._interact_params and self._interact_params.timer >= SYSHUD._settings.min_timer_duration then
            return false
        end
    end
...
function PlayerStandard:_check_use_item(t, input)
    if SYSHUD._settings.equipment then
